...
public class GenerateMazeDelayed : MonoBehaviour {
    ...
    IEnumerator Start() // Delayed maze using coroutine so the progress can be viewed frame by frame
    {
        ...
        yield return StartCoroutine(generateWall()); // first step is generating full wall on spherical surface
        ...
        yield return StartCoroutine(visit(0, 0)); // begin recursive backtracking algorithm

        yield return StartCoroutine(generatePassage()); // begin creating passage by disabling wall marked as passage

        yield return new WaitForSeconds(1);

        yield return StartCoroutine(generateShortcut()); // begin disabling some wall randomly
    ...
    private IEnumerator generatePassage()
    {
        for (int x = 0; x < sizeX; x++)
        {
            for (int y = 0; y < sizeY; y++)
    ...
    private IEnumerator generateShortcut()
    {
    ...
    private IEnumerator visit(int x, int y) // recursive method
    {
        ...
        foreach (int i in indexQueue)
        {
            ...
            else
            {
                ...
                yield return StartCoroutine(visit(newX, newY)); // recursively visit next neighbour
            }
        }
